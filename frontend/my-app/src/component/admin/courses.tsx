  ...
  const handleCreateContent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedContentType === 'pdf') {
        ...
        formData.append('estimated_duration', String(contentForm.time_limit || 15));
        formData.append('min_required_time', String(contentForm.max_attempts || 12));
        ...
        requestData = {
          ...
          estimated_duration: contentForm.time_limit || 15,
          min_required_time: contentForm.max_attempts || 12
        };
      ...
      // Refresh modules data to get updated time calculations
      const modulesResponse = await api.get(`courses/${courseId}/modules/`);
      
      // Also refresh the course data to get updated course-level times
      ...
      alert('Content created successfully! Module and course times updated automatically.');
    } catch (error: any) {
    ...
    try {
      ...
      if (editingContent.content_type.toLowerCase() === 'pdf') {
        ...
        if (contentForm.file) {
          ...
          formData.append('title', contentForm.title);
          formData.append('caption', contentForm.caption || '');
          formData.append('estimated_duration', String(contentForm.time_limit || 0));
          formData.append('min_required_time', String(contentForm.max_attempts || 0));
          formData.append('pdf_file', contentForm.file);
          requestData = formData;
          headers = { 'Content-Type': 'multipart/form-data' };
        } else {
          requestData = {
            title: contentForm.title,
            caption: contentForm.caption || '',
            estimated_duration: contentForm.time_limit || 0,
            min_required_time: contentForm.max_attempts || 0
          };
          headers = { 'Content-Type': 'application/json' };
        }
